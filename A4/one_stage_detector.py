...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channels in stem_channels:
            conv_cls = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
            nn.init.normal_(conv_cls.weight, mean=0, std=0.01)
            stem_cls.append(conv_cls)
            stem_cls.append(nn.ReLU())
            conv_box = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(in_channels, num_classes, kernel_size=3, stride=1, padding=1)  # Class prediction conv
        nn.init.normal_(self.pred_cls.weight, 0, 0.01)
        nn.init.constant_(self.pred_cls.bias, 0)

        self.pred_box = nn.Conv2d(in_channels, 4, kernel_size=3, stride=1, padding=1)  # Box regression conv
        nn.init.normal_(self.pred_box.weight, 0, 0.01)
        nn.init.constant_(self.pred_box.bias, 0)

        self.pred_ctr = nn.Conv2d(in_channels, 1, kernel_size=3, stride=1, padding=1) # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feats in feats_per_fpn_level.items():
            # Forward pass through stem layers.
            feats_cls = self.stem_cls(feats)
            feats_box = self.stem_box(feats)

            # Forward pass through prediction layers.
            class_logits[level_name] = self.pred_cls(feats_cls)
            boxreg_deltas[level_name] = self.pred_box(feats_box)
            centerness_logits[level_name] = self.pred_ctr(feats_box)

            #reshape the output
            class_logits[level_name] = class_logits[level_name].permute(0, 2, 3, 1).flatten(1, 2)
            boxreg_deltas[level_name] = boxreg_deltas[level_name].permute(0, 2, 3, 1).flatten(1, 2)
            centerness_logits[level_name] = centerness_logits[level_name].permute(0, 2, 3, 1).flatten(1, 2)
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        centers = centers.to(gt_boxes.device)
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    deltas = torch.zeros(gt_boxes.shape[0], 4, device=gt_boxes.device, dtype=gt_boxes.dtype)
    ...
    # assign delta of background to -1
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # if deltas are all -1, centerness should be -1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        shapes_per_fpn_level = {'p3' : feats_per_fpn_level['p3'].shape,
                                'p4' : feats_per_fpn_level['p4'].shape,
                                'p5' : feats_per_fpn_level['p5'].shape}
        # print(shapes_per_fpn_level)
        locations_per_fpn_level = get_fpn_location_coords(
            shapes_per_fpn_level, self.backbone.fpn_strides, device=images.device
        )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(gt_boxes.shape[0]):
            match_gt_box_batch = fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes[i])
            matched_gt_boxes.append(match_gt_box_batch)
        ...
        for i in range(gt_boxes.shape[0]):
            ...
            for level_name in locations_per_fpn_level.keys():
                delta_dict[level_name] = fcos_get_deltas_from_locations(locations_per_fpn_level[level_name], matched_gt_boxes[i][level_name], self.backbone.fpn_strides[level_name])
        ...
        # Get the matched GT class labels for every location and turn into one-hot. number of classes = pred_cls_logits.shape[2]
        # matched_gt_cls = matched_gt_boxes[:, :, 4].long()
        # matched_gt_cls = F.one_hot(matched_gt_cls, pred_cls_logits.shape[2]).to(torch.float32)
        # loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=matched_gt_cls)


        # loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduce="None")
        # loss_box[matched_gt_deltas < 0] = 0 # set loss to zero for background positions
        # loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, fcos_make_centerness_targets(matched_gt_deltas), reduce="None")
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_classes = classes[idx]

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[idx], 
                                                              level_locations[idx], self.backbone.fpn_strides[level_name])
