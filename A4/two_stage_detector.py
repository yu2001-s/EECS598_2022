...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for stem_channel in stem_channels:
            conv = nn.Conv2d(prev_channels, stem_channel, kernel_size=3, stride=1, padding=1)
            relu = nn.ReLU()
            #initilize weights
        ...
        # Replace "pass" statement with your code
        self.pred_obj = nn.Conv2d(prev_channels, num_anchors, kernel_size=1, stride=1)
        nn.init.normal_(self.pred_obj.weight, mean=0, std=0.01)
        nn.init.zeros_(self.pred_obj.bias)

        self.pred_box = nn.Conv2d(prev_channels, num_anchors * 4, kernel_size=1, stride=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feats in feats_per_fpn_level.items():
            # Pass through stem layers:
            feats = self.stem_rpn(feats)

            # Pass through prediction layers:
            object_logits[level_name] = self.pred_obj(feats) # (batch_size, num_anchors, H, W)
            object_logits[level_name] = object_logits[level_name].permute(0, 2, 3, 1).contiguous().view(batch_size, -1) # (batch_size, H * W * num_anchors)

            boxreg_deltas[level_name] = self.pred_box(feats) # (batch_size, num_anchors * 4, H, W)
            boxreg_deltas[level_name] = boxreg_deltas[level_name].permute(0, 2, 3, 1).contiguous().view(batch_size, -1, 4) # (batch_size, H * W * num_anchors, 4)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    inter = torch.clamp((rb - lt), min=0).prod(dim=2)
...
def rcnn_get_deltas_from_anchors(
    ...
    # Set deltas of "background/neutral" GT boxes to -1e8
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        location = get_fpn_location_coords(shape_per_fpn_level, strides_per_fpn_level, 
                                           device=feats_per_fpn_level["p3"].device)
        
        anchors_per_fpn_level = generate_fpn_anchors(location, 
        ...
        for image_idx in range(num_images):
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes, gt_boxes[image_idx], self.anchor_iou_thresholds))
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            # print("nms_topk ", self.pre_nms_topk)
            nms_topk = min(self.pre_nms_topk, level_obj_logits.shape[1])
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                proposal_boxes[:, 0] = torch.clamp(proposal_boxes[:, 0], min=0, max=image_size[0])
                proposal_boxes[:, 1] = torch.clamp(proposal_boxes[:, 1], min=0, max=image_size[1])
                proposal_boxes[:, 2] = torch.clamp(proposal_boxes[:, 2], min=0, max=image_size[0])
                proposal_boxes[:, 3] = torch.clamp(proposal_boxes[:, 3], min=0, max=image_size[1])

                
                _, topk_indices = torch.topk(level_obj_logits[_batch_idx], nms_topk)
                topk_proposal_boxes = proposal_boxes[topk_indices]
                objectness = level_obj_logits[_batch_idx][topk_indices]

                keep_indices = torchvision.ops.nms(topk_proposal_boxes, objectness, self.nms_thresh)

                level_proposals_per_image.append(proposal_boxes[keep_indices])
                
                # if _batch_idx == level_obj_logits.shape[0]-1:
                #     print(f"leven {level_name} end\n")
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        linear = nn.Linear(prev_channels * self.roi_size[0] * self.roi_size[1], self.num_classes + 1)
    ...
    def forward(
        ...
        sampled_gt_cls = (matched_gt_boxes[idx, 4].long() + 1)
        pred_cls_logits = pred_cls_logits[idx]


        loss_cls = F.cross_entropy(pred_cls_logits, sampled_gt_cls)
