...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        ...
        for j in range(K):
            weight[j] = torch.dot(query[i], key[j]) / torch.sqrt(torch.tensor(M))
        weight = F.softmax(weight, dim=0)
        out[i] = torch.matmul(weight, value)
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(K):
        ...
        for j in range(K):
            weight[:,j] = torch.mul(query[:,i,:], key[:,j,:]).sum(dim=1) / torch.sqrt(torch.tensor(M))
        weight = F.softmax(weight, dim=1)
        out[:,i,:] = torch.bmm(weight.unsqueeze(1), value).squeeze(1)
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scaled_dot_product = torch.bmm(query, key.transpose(-2, -1)) / math.sqrt(M)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for _ in range(num_heads):
            self.attention_layers.append(SelfAttention(dim_in, dim_out, dim_out))
        self.linear = nn.Linear(dim_out * num_heads, dim_in)
        c = torch.sqrt(torch.tensor(6 / (dim_in + dim_out * num_heads)))
        self.linear.weight.data.uniform_(-c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = torch.cat([layer(query, key, value, mask) for layer in self.attention_layers], dim=-1)
        y = self.linear(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        mean = x.mean(dim=-1, keepdim=True)  # Compute the mean across the embedding dimension
        variance = x.var(dim=-1, unbiased = False, keepdim=True)  # Compute the variance across the embedding dimension
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        first_mlp = nn.Linear(inp_dim, hidden_dim_feedforward)
        second_mlp = nn.Linear(hidden_dim_feedforward, inp_dim)
        c = torch.sqrt(torch.tensor(6 / (inp_dim + hidden_dim_feedforward)))
        first_mlp.weight.data.uniform_(-c, c)
        second_mlp.weight.data.uniform_(-c, c)

        self.first_mlp = first_mlp
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.muti_head_attention = MultiHeadAttention(num_heads, emb_dim, int(emb_dim / num_heads))
    ...
    def forward(self, x):
        ...
        out1 = self.muti_head_attention(x, x, x)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.triu(torch.ones((N, K, K), dtype=torch.bool), diagonal=1).to(seq.device)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = pos.repeat(M, 1).transpose(0, 1).unsqueeze(0)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    y = torch.zeros(K, M).float()
    for i in range(math.ceil(M/2)):
        a = math.floor((2 * i) / M)
        # a = (2 * i) / M
        a = 1 / (10000 ** a)
        y[:, i*2] = torch.sin(pos / a)
        if i*2 + 1 < M:
            y[:, i*2 + 1] = torch.cos(pos / a)
