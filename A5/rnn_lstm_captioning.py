...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = (1 - torch.tanh(x @ Wx + prev_h @ Wh + b) ** 2) * dnext_h
    dx = dtanh @ Wx.T
    dprev_h = dtanh @ Wh.T
    dWx = x.T @ dtanh
    dWh = prev_h.T @ dtanh
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h, cache_t = rnn_step_forward(x[:, t, :], pred_h, Wx, Wh, b)
        h[:, t, :] = next_h
        pred_h = next_h
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    #F.cross_entropy, the shape should be (N, C, *), where N is the batch size, C
    #is the number of classes, and * means any number of additional dimensions.
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.image_encoder = ImageEncoder(pretrained=image_encoder_pretrained, verbose=True)

        #project the image feature to the initial hidden state $h0$ (for RNN/LSTM, of shape (N, H)) or
        if cell_type == "rnn" or cell_type == "lstm":
            self.average_pool = nn.AvgPool2d(kernel_size=4) # N, C
            self.affine_transform = nn.Linear(self.image_encoder.out_channels, hidden_dim) # C to H
        else:
            self.affine_transform = nn.Conv2d(self.image_encoder.out_channels, hidden_dim, kernel_size=1, stride=1, padding=0) # C to H
        ...
        self.linear = nn.Linear(hidden_dim, vocab_size) # H to V
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "rnn" or self.cell_type == "lstm":
            # (1) Use an affine transformation to project the image feature to shape (N, H)
            h0 = self.average_pool(img_features).squeeze()
            h0 = self.affine_transform(h0)
            # (2) Use a word embedding layer to transform the words in captions_in
            captions_in_embed = self.word_embedding(captions_in)
            # (3) Use either a vanilla RNN or LSTM
            hidden_states = self.rnn(captions_in_embed, h0)
        else:
            # (1) Use an affine transformation to project the image feature to shape (N, H, 4, 4)
            h0 = self.affine_transform(img_features)
            # (2) Use a word embedding layer to transform the words in captions_in
            captions_in_embed = self.word_embedding(captions_in)
            # (3) Use Attention LSTM
            hidden_states = self.rnn(captions_in_embed, h0)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "rnn":
            ...
            for t in range(1, max_length):
                prev_word_embed = self.word_embedding(captions[:, t - 1])
        ...
        if self.cell_type == "attn":
            # (1) Embed the previous word using the learned word embeddings
            captions[:, 0] = self._start
            A = self.affine_transform(img_features)
            h0 = A.mean(dim=(2, 3))
            c0 = A.mean(dim=(2, 3))
            # (2) Make an RNN step using the previous hidden state and the embedded
            #     current word to get the next hidden state.
            ...
            for t in range(1, max_length):
                prev_word_embed = self.word_embedding(captions[:, t - 1])
                attn, atten_weights = dot_product_attention(prev_h, A)
                attn_weights_all[:, t] = atten_weights
                next_h, next_c = self.rnn.step_forward(prev_word_embed, prev_h, prev_c, attn)
                # (3) Apply the learned affine transformation to the next hidden state to
                #     get scores for all words in the vocabulary
                scores = self.linear(next_h)
                # (4) Select the word with the highest score as the next word, writing it
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        input_gate = torch.sigmoid(x @ self.Wx[:, :prev_h.shape[1]] + prev_h @ self.Wh[:, :prev_h.shape[1]] + self.b[:prev_h.shape[1]])
        forget_gate = torch.sigmoid(x @ self.Wx[:, prev_h.shape[1]:prev_h.shape[1] * 2] + prev_h @ self.Wh[:, prev_h.shape[1]:prev_h.shape[1] * 2] + self.b[prev_h.shape[1]:prev_h.shape[1] * 2])
        output_gate = torch.sigmoid(x @ self.Wx[:, prev_h.shape[1] * 2:prev_h.shape[1] * 3] + prev_h @ self.Wh[:, prev_h.shape[1] * 2:prev_h.shape[1] * 3] + self.b[prev_h.shape[1] * 2:prev_h.shape[1] * 3])
        gate_gate = torch.tanh(x @ self.Wx[:, prev_h.shape[1] * 3:] + prev_h @ self.Wh[:, prev_h.shape[1] * 3:] + self.b[prev_h.shape[1] * 3:])
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        # Replace "pass" statement with your code
        T = x.shape[1]
        hn = torch.zeros((x.shape[0], T, h0.shape[1]), dtype=x.dtype, device=x.device)
        prev_h = h0
        prev_c = c0
        for t in range(T):
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Reshape A to (N, 4*4, H) for batched matrix multiplication
    
    A = A.reshape(N, H, -1) # N, H, 16

    attn_weights = torch.bmm(A.transpose(1,2),  prev_h.unsqueeze(dim=2)) / math.sqrt(H) # N, 16
    attn_weights = F.softmax(attn_weights, dim=1) # N, 16


    attn = torch.bmm(A, attn_weights) # N, H
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h, next_c = None, None
        # Replace "pass" statement with your code
        input_gate = torch.sigmoid(x @ self.Wx[:, :prev_h.shape[1]] + prev_h @ self.Wh[:, :prev_h.shape[1]] + attn @ self.Wattn[:, :prev_h.shape[1]] + self.b[:prev_h.shape[1]])
        forget_gate = torch.sigmoid(x @ self.Wx[:, prev_h.shape[1]:prev_h.shape[1] * 2] + prev_h @ self.Wh[:, prev_h.shape[1]:prev_h.shape[1] * 2] + attn @ self.Wattn[:, prev_h.shape[1]:prev_h.shape[1] * 2] + self.b[prev_h.shape[1]:prev_h.shape[1] * 2])
        output_gate = torch.sigmoid(x @ self.Wx[:, prev_h.shape[1] * 2:prev_h.shape[1] * 3] + prev_h @ self.Wh[:, prev_h.shape[1] * 2:prev_h.shape[1] * 3] + attn @ self.Wattn[:, prev_h.shape[1] * 2:prev_h.shape[1] * 3] + self.b[prev_h.shape[1] * 2:prev_h.shape[1] * 3])
        gate_gate = torch.tanh(x @ self.Wx[:, prev_h.shape[1] * 3:] + prev_h @ self.Wh[:, prev_h.shape[1] * 3:] + attn @ self.Wattn[:, prev_h.shape[1] * 3:] + self.b[prev_h.shape[1] * 3:])
